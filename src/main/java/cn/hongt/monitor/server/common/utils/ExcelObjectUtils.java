...
public class ExcelObjectUtils implements Closeable {
    ...
    public ExcelObjectUtils() {
        this.workbook = new HSSFWorkbook();
        this. sheet = this.workbook.createSheet("sheet1");
    }

    public ExcelObjectUtils(String sheetName) {
        this.workbook = new HSSFWorkbook();
        this.sheet = this.workbook.createSheet(sheetName);
        sheet.setDefaultRowHeight((short) (2 * 256)); //设置默认行高，表示2个字符的高度
        sheet.setDefaultColumnWidth(20);    //设置默认列宽，实际上回多出2个字符，不知道为什么
//        sheet.setColumnWidth(0,1500); // 设置指定列宽
     ...
     * @remark:
     * */
    public void buildExcelFile(String path) throws Exception{
        // 【修复#47】try-with-resources确保异常时也能关闭流
     ...
     * @remark:
     * */
    public void buildExcelDocument(String filename, HttpServletResponse response, HttpServletRequest request) throws Exception{
        // 【修复#49】getHeader可能返回null，加空判断防止NPE
        ...
        // 【修复#48】Servlet容器管理OutputStream生命周期，不应手动close
